...
import { VapixCore, RequestOptions } from './vapix/core';
import { listParams, ParamSet } from './vapix/params';
import { getApiList, getDeviceInfo, deviceInfoFromParams, ApiEntry, DeviceInfo } from './vapix/deviceinfo';
import { getSupportedSdks } from './vapix/apps';
import { readPtzSupport, PtzSupport } from './vapix/ptz';
import { supportsLocalStorage } from './vapix/storage';
import { FirmwareVersion, acapGeneration, atLeast, osTrack, parseFirmware } from './vapix/firmware';

export type EventTransport = 'websocket' | 'soap' | 'rtsp-metadata';

export type Capabilities = {
    firmware: FirmwareVersion;
    osTrack: string | null;
    acapGeneration: { label: string; docs: string } | null;
    device: DeviceInfo;

    /** Raw Brand + Properties, so commands can read anything else they need. */
    params: ParamSet;
    /** apidiscovery.cgi output when available (AXIS OS 8.50+), else null. */
    apis: Map<string, ApiEntry> | null;

    ptz: PtzSupport;

    acap: {
        /** Properties.EmbeddedDevelopment.Version — null means no ACAP support. */
        embeddedDevelopmentVersion: string | null;
        /** applications/list.cgi requires EmbeddedDevelopment 1.20 or later. */
        canList: boolean;
        /** applications/config.cgi — AXIS OS 11.2 and later. */
        canConfig: boolean;
        /** From applications/info.cgi, when the device offers it. */
    ...
    events: {
        /** Which subscription transports this device should be able to do. */
        transports: EventTransport[];
        websocket: boolean;
        /** GetEventInstances over /vapix/services — firmware 5.50 and later. */
        soap: boolean;
    };

    apiDiscovery: boolean;
    basicDeviceInfo: boolean;
 ...
 */
function discovered(apis: Map<string, ApiEntry> | null, id: string): boolean | null {
 ...
 */
function detectWebsocketEvents(apis: Map<string, ApiEntry> | null, firmware: FirmwareVersion): boolean {
    if (discovered(apis, API_IDS.websocketEvents) === true) return true;
    return atLeast(firmware, '10.11') !== false;
}

export async function detectCapabilities(core: VapixCore, options: RequestOptions = {}): Promise<Capabilities> {
    ...
    const params = await listParams(core, ['Brand', 'Properties'], options);

    const firmware = parseFirmware(params.get('Properties.Firmware.Version'));
    ...
    const [apis, device, supportedSdks] = await Promise.all([
        getApiList(core, options).catch(() => null),
        getDeviceInfo(core, { ...options, params }).catch(() => deviceInfoFromParams(params)),
        ...
        params.has('Properties.EmbeddedDevelopment.Version')
            ? getSupportedSdks(core, options).catch(() => null)
    ...
    const soap = atLeast(firmware, '5.50') !== false;
    ...
    if (websocket) transports.push('websocket');
    if (soap) transports.push('soap');
    if (rtsp) transports.push('rtsp-metadata');

    return {
        firmware,
        osTrack: osTrack(firmware),
        acapGeneration: acapGeneration(firmware),
        device,
        params,
        apis,
        ptz: readPtzSupport(params),
        acap: {
            ...
            canList:
                embeddedDevelopmentVersion !== null &&
                atLeast(parseFirmware(embeddedDevelopmentVersion), '1.20') === true,
            ...
            canConfig: atLeast(firmware, '11.2') !== false,
            supportedSdks,
        },
        storage: {
            supported: supportsLocalStorage(params),
        ...
        firmwareManagement:
            params.has('Properties.FirmwareManagement.Version') || discovered(apis, API_IDS.firmwareManagement) === true,
    };
}
 ...
 */
export function compatibilityNotes(caps: Capabilities): string[] {
    const notes: string[] = [];
    const fw = caps.firmware;

    if (fw.parts.length === 0) {
        notes.push(
            'This device did not report a parseable firmware version, so version-gated features are ' +
                'attempted rather than pre-checked. Failures will come from the camera, not from this CLI.'
        );
    }

    if (atLeast(fw, '11.6') === true) {
        notes.push(
            'AXIS OS 11.6 and later ship without a default "root" account. If authentication fails, the ' +
                'camera may still need its first user created in the web UI.'
        );
    }

    if (atLeast(fw, '12.0') === true) {
        notes.push(
            'AXIS OS 12.0 removed TLS 1.0/1.1, refuses unsigned ACAPs by default, and dropped ' +
                'getBrand.cgi and releaseinfo.cgi (this CLI uses basicdeviceinfo.cgi and param.cgi instead).'
        );
    }

    if (atLeast(fw, '12.1') === true) {
        notes.push(
            'From AXIS OS 12.1 the default authentication policy uses Basic over HTTPS and Digest over ' +
    ...
    if (!caps.events.websocket) {
        notes.push(
            'Live event streaming over WebSocket needs AXIS OS 10.11 or later. On this device use ' +
    ...
    if (caps.ptz.digital && !caps.ptz.mechanical) {
        notes.push(
            'This camera has digital PTZ only. Position queries will report zoom but no pan or tilt, ' +
    ...
    if (caps.acap.embeddedDevelopmentVersion === null) {
        ...
        notes.push(
            `This device reports ACAP support version ${caps.acap.embeddedDevelopmentVersion}; listing ` +
