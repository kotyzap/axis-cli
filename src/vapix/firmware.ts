...
export type FirmwareVersion = {
    /** Raw string exactly as the camera reported it. */
    raw: string;
    /** Numeric components, e.g. [10, 12, 338]. Empty when unparseable. */
    parts: number[];
    major: number | null;
    minor: number | null;
};

export function parseFirmware(raw: string | undefined | null): FirmwareVersion {
    const text = (raw ?? '').trim();
 ...
 */
export function atLeast(fw: FirmwareVersion, threshold: string): boolean | null {
    if (fw.parts.length === 0) return null;
 ...
 */
export function osTrack(fw: FirmwareVersion): string | null {
    if (fw.major === null) return null;
    switch (fw.major) {
        case 12:
 ...
 */
export function acapGeneration(fw: FirmwareVersion): { label: string; docs: string } | null {
    if (fw.major === null) return null;
    if (fw.major >= 12) {
        return {
            label: `ACAP ${fw.major}`,
