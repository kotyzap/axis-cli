...
import { action, info as logInfo, isJsonMode, printJson } from '../output';
import { describeSdCard, listDisks } from '../vapix/storage';
import { tryListParams } from '../vapix/params';
    ...
    program
        ...
        .action(
            action(async (name: string, opts) => {
                ...
                let acaps: { name: string; niceName: string; version: string; status: string; vendor: string }[] | null =
                    null;
                let acapError: string | null = null;
                if (caps.acap.canList) {
                    try {
                        ...
                        acapError = (err as Error).message;
                    }
                } else {
                    acapError =
                ...
                const network = await tryListParams(cam.core, ['Network.eth0']);
                ...
                line(
                    'Firmware:',
                    d.firmwareVersion ? `${d.firmwareVersion}${caps.osTrack ? ` — ${caps.osTrack}` : ''}` : null
                );
                ...
                line('IP (eth0):', ip);
                line('SD card:', describeSdCard(storage));

                if (acaps === null) {
                    logInfo(`  (ACAP list unavailable — ${acapError})`);
                    return;
                }
                ...
                if (acaps.length === 0) {
                    ...
                    if (!opts.allAcaps) console.log('    (re-run with --all-acaps to list every installed ACAP)');
                    return;
                }
                console.log(`  ${label}:`);
