 ...
 */
export function printTable(head: string[], rows: (string | number)[][], jsonRows?: unknown) {
    if (jsonMode) {
        printJson(jsonRows ?? rows.map((r) => Object.fromEntries(r.map((v, i) => [head[i], v]))));
        return;
    }
    const table = new Table({ head });
    for (const r of rows) table.push(r.map((v) => String(v)));
    console.log(table.toString());
}

/**
 * Undici surfaces every transport failure as a bare "fetch failed"; the useful
 * detail (ECONNREFUSED, ETIMEDOUT, self-signed cert, ...) is on `.cause`.
 */
type ZodIssue = { path?: (string | number)[]; message?: string; received?: string };
 ...
 */
function describeZodError(err: Error): string | null {
    let issues: ZodIssue[] | null = (err as Error & { issues?: ZodIssue[] }).issues ?? null;
    ...
    const parts = issues.map((i) => {
        ...
        if (i.received === 'nan') return `${field} is not reported by this camera`;
 ...
 */
function describeUnsupported(err: Error): string | null {
    if (err.name !== 'UnsupportedError') return null;
    const detail = (
        err as Error & {
            detail?: { requires?: string; firmware?: { raw: string }; docs?: string; alternative?: string };
        }
    ).detail;

    const lines = [err.message];
    if (detail?.requires) lines.push(`  Requires:    ${detail.requires}`);
    if (detail?.firmware?.raw) lines.push(`  This camera: firmware ${detail.firmware.raw}`);
    if (detail?.alternative) lines.push(`  Try instead: ${detail.alternative}`);
    if (detail?.docs) lines.push(`  Reference:   ${detail.docs}`);
    return lines.join('\n');
}
...
export function describeError(err: Error): string {
    ...
    if ((err as Error & { name?: string }).name === 'TimeoutError' || /aborted due to timeout/i.test(err.message)) {
        return (
            `${err.message} — the camera didn't respond in time. ` +
    ...
    const hint: Record<string, string> = {
        ...
        DEPTH_ZERO_SELF_SIGNED_CERT:
            'the camera uses a self-signed certificate — re-add the profile with --tls-insecure, or run "axis camera update <name> --tls-insecure"',
...
export function action(fn: (...args: any[]) => Promise<void> | void) {
    return async (...args: any[]) => {
        try {
            ...
            fail(describeError(err as Error));
            process.exitCode = 1;
        }
    };
}
