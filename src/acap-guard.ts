...
type FamilyId = 'CamOverlay' | 'CamStreamer' | 'CamSwitcher' | 'CamScripter';

const INSTALL_HINT: Record<FamilyId, string> = {
 ...
 */
export async function withAcap<T>(cam: CameraSession, familyId: FamilyId, fn: () => Promise<T>): Promise<T> {
    try {
        return await fn();
    } catch (err) {
        ...
        if (caps && caps.acap.embeddedDevelopmentVersion === null) {
            throw new UnsupportedError(
                `Camera "${cam.name}" cannot run ACAPs, so ${familyId} cannot be installed on it and ` +
                    `the ${familyId.toLowerCase()} commands do not apply to this device.`,
                { firmware: caps.firmware, alternative: `axis caps ${cam.name}` }
            );
        }
        ...
        if (status === null) {
            ...
            throw new UnsupportedError(
                `The ${familyId} API on "${cam.name}" did not respond, and the installed-application list ` +
                    `could not be read to find out why${caps ? ` (firmware ${caps.firmware.raw || 'unknown'})` : ''}.\n` +
                    `  The underlying error was: ${(err as Error).message}`,
                { alternative: `axis caps ${cam.name}` }
            );
        }

        if (!status.installed) {
            throw new UnsupportedError(
                `${familyId} is not installed on "${cam.name}", so there is no ${familyId} API to talk to.`,
                {
        ...
        if (!status.running) {
            throw new UnsupportedError(
                `${familyId}${status.version ? ` ${status.version}` : ''} is installed on "${cam.name}" ` +
